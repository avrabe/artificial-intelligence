...
SELF_AGENTS = {
    "RANDOM": Agent(RandomPlayer, "Random Agent 1"),
    "GREEDY": Agent(GreedyPlayer, "Greedy Agent 1"),
    "MINIMAX": Agent(MinimaxPlayer, "Minimax Agent 1"),
    "SELF": Agent(CustomPlayer, "Custom TestAgent 1")
}

Match = namedtuple("Match", "players initial_state time_limit match_id debug_flag")


class OpenBookIsolation(Isolation):
    pass
...
def main(args):
    ...
    custom_agent = SELF_AGENTS[args.self.upper()]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-s', '--self', type=str, default='RANDOM', choices=list(SELF_AGENTS.keys()),
        help="""\
            self player
        """
    )
